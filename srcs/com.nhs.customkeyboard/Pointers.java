...
public final class Pointers implements Handler.Callback
{
  ...
  public void onTouchDown(float x, float y, int pointerId, KeyboardData.Key key)
  {
    ...
    boolean shiftPressed = mods.has(KeyValue.Modifier.SHIFT);

    KeyValue value = key.getOutputValue(0, shiftPressed);
    if (value == null)
      value = _handler.modifyKey(key.keys[0], mods);
    else if (key.keyOutputs == null || key.keyOutputs[0] == null)
  ...
  private KeyValue getNearestKeyAtDirection(Pointer ptr, int direction)
  {
    ...
    for (int i = 0; i > -4; i = (~i>>31) - i)
    {
      ...
      KeyValue raw = getKeyAtDirection(ptr.key, d);
      if (raw == null)
        continue;

      KeyValue shifted = ptr.key.getOutputValue(DIRECTION_TO_INDEX[d], ptr.modifiers.has(KeyValue.Modifier.SHIFT));

      if (ptr.modifiers.has(KeyValue.Modifier.SHIFT)
              && shifted != null
              && ptr.key.keyOutputs != null
              && ptr.key.keyOutputs[DIRECTION_TO_INDEX[d]] != null)
      {
        k = shifted;
      }
      else
      {
        k = _handler.modifyKey(raw, ptr.modifiers);
      }
      if (k != null)
      {
  ...
  {
    Pointer ptr = getPtr(pointerId);
    if (ptr == null || ptr.longPressHandled)
      return;
    if (ptr.hasFlagsAny(FLAG_P_SLIDING))
    {
  ...
  private void handleLongPress(Pointer ptr)
  {
    ...
    if (ptr.key != null && ptr.key.longPressKey != null && ptr.key.longPressKey.getKind() == KeyValue.Kind.Event)
    {
      ptr.longPressHandled = true;
      _handler.onPointerUp(ptr.key.longPressKey, ptr.modifiers);
      _handler.onPointerFlagsChanged(true);
      return;
    }

    boolean isNormalTextKey = ptr.key != null
        && ptr.key.role == KeyboardData.Key.Role.Normal
        && ptr.value != null
        && !ptr.value.hasFlagsAny(KeyValue.FLAG_SPECIAL)
        && (ptr.value.getKind() == KeyValue.Kind.Char || ptr.value.getKind() == KeyValue.Kind.String);

    if (_config.longpress_custom_char_enabled && isNormalTextKey)
    {
      KeyValue lpKey = ptr.key.longPressKey;
      if (lpKey == null)
      {
        if (ptr.key.keys[7] != null)
          lpKey = ptr.key.keys[7];
        else if (ptr.key.keys[2] != null)
          lpKey = ptr.key.keys[2];
      }
      if (lpKey != null && !lpKey.hasFlagsAny(KeyValue.FLAG_SPECIAL))
      {
        ptr.longPressHandled = true;
        KeyValue finalValue = _handler.modifyKey(lpKey, ptr.modifiers);
        ptr.value = finalValue;
        _handler.onPointerUp(finalValue, ptr.modifiers);
        _handler.onPointerFlagsChanged(true);
        return;
      }
    }
