...
public final class LayoutModifier
{
  ...
  static KeyboardData.Row bottom_row_period;
  ...
  public static KeyboardData modify_layout(KeyboardData kw)
  {
    ...
    else if (globalConfig.add_number_row && !kw.embedded_number_row) // The numpad removes the number row
    {
      ...
      {
        added_number_row = added_number_row.with_height(added_number_row.height * globalConfig.number_row_height_scale);
      }
    ...
    if (kw.bottom_row)
    {
      KeyboardData.Row br = (globalConfig != null && globalConfig.period_key_bottom_row && bottom_row_period != null)
          ? bottom_row_period
          : bottom_row;
      kw = kw.insert_row(br, kw.rows.size());
    }
  ...
  /** Modify period symbols layout without stripping keys that match number row symbols. */
  public static KeyboardData modify_period_symbols(KeyboardData kw)
  {
    if (globalConfig != null && globalConfig.add_number_row && !kw.embedded_number_row)
    {
      KeyboardData.Row added_number_row = modify_number_row(
          globalConfig.number_row_symbols ? number_row_symbols : number_row_no_symbols, kw);
      if (globalConfig.number_row_height_scale > 0f)
      {
        added_number_row = added_number_row.with_height(added_number_row.height * globalConfig.number_row_height_scale);
      }
    ...
    if (globalConfig != null && globalConfig.split_layout)
      kw = LayoutLandscapeModifier.transform_to_landscape(kw);
    return kw;
  }
  ...
  public static void init(Config globalConfig_, Resources res)
  {
    ...
    try
    {
      ...
      bottom_row_period = KeyboardData.load_row(res, R.xml.bottom_row_period);
