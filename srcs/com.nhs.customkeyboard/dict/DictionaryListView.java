...
import com.nhs.customkeyboard.DeviceLocales;
...
public class DictionaryListView extends LinearLayout
{
  ...
  public void refresh_card_backgrounds()
  {
    ...
    for (int i = 0; i < count; i++)
    {
      ...
      if (isFirst && isLast)
        child.setBackgroundResource(R.drawable.bg_settings_card_single);
      else if (isFirst)
        child.setBackgroundResource(R.drawable.bg_settings_card_top);
      else if (isLast)
        child.setBackgroundResource(R.drawable.bg_settings_card_bottom);
      else
        child.setBackgroundResource(R.drawable.bg_settings_card_middle);

      View divider = child.findViewById(R.id.card_divider);
      if (divider != null)
      {
  ...
  final class DictView implements View.OnClickListener
  {
    ...
    {
      boolean is_pending = _pending.contains(dict_name);
      boolean is_installed = installed.contains(dict_name);
      if (is_pending)
      {
        download_button.setImageDrawable(null);
      ...
      else
      {
        ...
        if (is_installed)
        {
          ...
          download_button.setImageTintList(
            ColorStateList.valueOf(ContextCompat.getColor(getContext(), R.color.settings_error)));
        }
        else
        {
          download_button.setImageResource(R.drawable.ic_download);
          download_button.setImageTintList(
            ColorStateList.valueOf(ContextCompat.getColor(getContext(), R.color.settings_primary)));
        }
      }
    }
