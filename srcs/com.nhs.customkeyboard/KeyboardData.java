...
public final class KeyboardData
{
  ...
  private static KeyboardData parse_keyboard(XmlPullParser parser) throws Exception
  {
    ...
    String dictionary = parser.getAttributeValue(null, "dictionary");
    ...
    return new KeyboardData(rows, kw, modmap, script, numpad_script, name, dictionary, keymap, swipekeymap, bottom_row, embedded_number_row, locale_extra_keys);
  }
  ...
  public KeyboardData with_rows(List<Row> rows_)
  {
    return new KeyboardData(rows_, compute_max_width(rows_), modmap, script,
            numpad_script, name, dictionary, keymap, swipekeymap, bottom_row, embedded_number_row,
            locale_extra_keys);
  }
  ...
  public static class Row
  {
    ...
    protected Row(List<Key> keys_, float h, float s)
    {
      ...
      keys = keys_;
      height = Math.max(h, keys_.size() == 0 ? 0.0f : 0.2f);
      shift = Math.max(s, 0f);
      keysWidth = kw;
    }
  ...
  public static class Key
  {
    ...
    public final KeyValue longPressKey;
    ...
    protected Key(
            ...
            Role role_)
    {
      this(ks, outs, antic, null, f, w, s, i, keyLabels, role_);
    }
    ...
    public static Key parse(XmlPullParser parser) throws Exception
    {
      ...
      String lp_val = get_key_attr(parser, "lp", "long_press");
      KeyValue longPressKey = null;
      if (lp_val != null)
      {
        String name_loc = stripPrefix(lp_val, "loc ");
        longPressKey = KeyValue.getKeyByName(name_loc != null ? name_loc : lp_val);
      }
