...
public class TaskerTriggerEngine
{
    ...
    /** Bounds how many trailing characters of an "nhck_patterns"
    ...
    /** Same length as [_config.expand_patterns] again - the
     ...
     checks before firing the %nhck_keyword_stop=true signal below. */
    private int[] _expand_live_matched_prefix_end = new int[0];
    /** Same length as [_config.expand_patterns] again - the
     [prefix_end] position a %nhck_keyword_stop=true call has already
    ...
    /** Call after EVERY committed character - whether typed forward
     ...
     for each configured "nhck_patterns" entry, finds the closest
                                      ...
                                      InputConnectionProvider late_conn_provider)
    {
        ...
        for (int i = 0; i < _config.expand_patterns.size(); i++)
        {
            ...
            {
                Log.w(LOG_TAG, "matching \"prefix\" failed for an nhck_patterns entry", e);
                continue;
            }
            ...
            for (int j = prefix_matches.size() - 1; j >= 0 && !acted; j--)
            {
                ...
                // still fires the one-off %nhck_keyword_stop signal
                ...
                // %nhck_keyword for the "stopped matching" branch below
                // when a newline is what ended this occurrence, since
                // sending a multi-line %nhck_keyword there would make
                // little sense.
                        ...
                        && _expand_regex_fail_fired_prefix_end[i] != prefix_end)
                {
                    ...
                    // time, with %nhck_keyword_stop=true, purely so it
    ...
    /** Fires [p.task] for one "nhck_patterns" occurrence. For a
     ...
     [check_expand_patterns] - sent as %nhck_keyword_stop so the task
