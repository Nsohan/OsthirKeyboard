...
public final class DirectBootAwarePreferences
{
  ...
  public static void copy_preferences_to_default_storage(Context context)
  {
    if (VERSION.SDK_INT >= 24)
    {
      try
      {
        SharedPreferences src = get_protected_prefs(context);
        SharedPreferences dst = PreferenceManager.getDefaultSharedPreferences(context);
        copy_shared_preferences(src, dst);
      }
