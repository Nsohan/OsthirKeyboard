...
$targetDevice = $Device
if (-not $targetDevice) {
    $connectedList = [System.Collections.Generic.List[string]]::new()
    foreach ($d in (Get-ConnectedDevices)) {
        if ($d -and "$d".Trim()) {
            $connectedList.Add("$d".Trim())
        }
    }
...
$adbArgs = if ($targetDevice) { @("-s", $targetDevice) } else { @() }
...
if ($LASTEXITCODE -ne 0) {
    ...
    adb @adbArgs uninstall com.nhs.customkeyboard.debug
    adb @adbArgs install -r $apkPath
}
...
adb @adbArgs shell am start -n com.nhs.customkeyboard.debug/com.nhs.customkeyboard.SettingsActivity | Out-Null
